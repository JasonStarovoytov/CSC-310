...
int main(int argc, char *argv[])
{
    ...
    int entryByte = sizeof(Sample), fileSize = 1;
    ...
    int indexArray[5];
    char newName[20];
    char fileStr[25] = "sorted_samples.bin";
    ...
    fileSize = filesystem::file_size(argv[1])/entryByte;

    mySample.fileSize = fileSize;
    mySample.entryByte = entryByte;
    ...
    {   
        inputBinary.read((char*) &smpl[i], entryByte);
        rangeEnd++;
    }
    ...
    for(int i = 0; i < fileSize; i++)
    {
        sortedBinary.write((char*) &smpl[i], entryByte);
    }
